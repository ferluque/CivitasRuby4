...
module Civitas
  class CivitasJuego
    ...
    def inicializar_mazo_sorpresas (tablero)
      ...
      @mazo.al_mazo(SorpresaPorJugador.new(20*(@jugadores.size()-1), "El jugador recibe 20 por cada jugador"))
      @mazo.al_mazo(SorpresaPagarCobrar.new(100, "Regalo de cumpleanios: el jugador recibe 100"))
      @mazo.al_mazo(SorpresaPorJugador.new(10*get_jugador_actual.cantidad_casas_hoteles, "El jugador recibe 10 de cada jugador"))
      @mazo.al_mazo(SorpresaPorCasaHotel.new(5*get_jugador_actual.cantidad_casas_hoteles, "El jugador recibe 5 por cada casa y por cada hotel"))
      @mazo.al_mazo(SorpresaCarcel.new(@tablero))
    end
  
    def inicializar_tablero (mazo)
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Calle Ganivet", 8.0, 1.1, 35.0, 50.0, 10.0)))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Calle Pedro Antonio de Alarcon", 12.0, 1.12, 45.0, 75.0, 25.0)))
      @tablero.aniade_casilla(CasillaSorpresa.new(mazo, "Casilla Sorpresa"))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Avenida Juan Pablo II", 17.0, 1.15, 75.0, 125.0, 35.0)))
      @tablero.aniade_casilla(CasillaImpuesto.new(50.0, "Impuesto de circulacion"))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Joaquina Eguaras", 25.0, 1.2, 85.0, 175.0, 50.0)))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Gran Via", 30.0, 1.25, 95.0, 200.0, 60.0)))
      @tablero.aniade_casilla(CasillaSorpresa.new(mazo, "Casilla Sorpresa"))
      #Se aniade automáticamente la carcel
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Avenida de Madrid", 35.0, 1.27, 105.0, 225.0, 65.0)))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Avenida de Andalucia", 40.0, 1.29, 120.0, 250.0, 80.0)))
      @tablero.aniade_casilla(CasillaImpuesto.new(150.0, "Impuesto de sociedades"))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Avenida de la Constitucion", 50.0, 1.31, 140.0, 275.0, 90.0)))
      @tablero.aniade_casilla(Casilla.new("Descanso"))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Avenida de Pulianas", 60.0, 1.33, 160.0, 300.0, 100.0)))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Acera del Darro", 75.0, 1.35, 170.0, 350.0, 110.0)))
      @tablero.aniade_casilla(CasillaImpuesto.new(200.0, "IRPF"))
      @tablero.aniade_casilla(CasillaSorpresa.new(mazo, "Casilla Sorpresa"))
      @tablero.aniade_casilla(CasillaCalle.new(TituloPropiedad.new("Paseo de los tristes",80.0, 1.34, 175.0, 315.0, 115.0)))
    end
